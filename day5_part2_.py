# move multiple crates at once CRATEMOVER9001
...
with open("day5_part1_input.txt") as f:
    ...
    for line in lines:
        ...
        # append specified quantity to move_to list
        moving = stacks[move_from][(len(stacks[move_from]) - quantity):]
        for box in moving:
            stacks[move_to].append(box)
        # remove specified quantity from move_from list
        stacks[move_from] = stacks[move_from][:len(stacks[move_from]) - quantity]
